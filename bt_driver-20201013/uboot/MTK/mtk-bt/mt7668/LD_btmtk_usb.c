...
//- Local Configuration -----------------------------------------------------
#define LD_VERSION "3.0.19102401"
...
static void btmtk_usb_cap_init(struct LD_btmtk_usb_data *data)
{
	...
	if (is_mt7662T(data) || is_mt7632T(data)) {
		...
		sprintf((char *)data->rom_patch_bin_file_name, "mt%04x_patch_e%x_hdr.bin",data->chip_id & 0xFFFF, (fw_ver & 0xFF) + 1);
...
int btmtk_usb_set_unify_woble(struct LD_btmtk_usb_data *data)
{
	...
	u8 cmd[] = { 0xC9, 0xFC, 0x14, 0x01, 0x20, 0x02, 0x00, 0x01, 0x02, 0x01,
		0x00, 0x05, 0x10, 0x01, 0x00, 0x40, 0x06, 0x02, 0x40, 0x5A, 0x02,
		0x41, 0x0F};
